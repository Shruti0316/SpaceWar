...
var frame=0,time=0,score=0,dx=4;
var aliens=[],gun=[];
...
alien1_img=new Image();
alien2_img=new Image();
alien1_img.src='sprite/alien(1).png';
alien2_img.src='sprite/alien(2).png';
bullet_img=new Image();
bullet_img.src="spirte/bullet.png";
...
clock();
document.getElementById('highscore').textContent = 'HighScore: '+localStorage.getItem("SpacewarHacker");;
...
function alien(y,num){
    ...
    this.attack=function(){
        ...
        ctx.drawImage(alien_img,this.x,this.y,50,50);}
            ...
            ctx.drawImage(alien1_img,this.x,this.y,50,50);
        }
        else{
            ctx.drawImage(alien2_img,this.x,this.y,50,50);
        }
    }
}
function bullet(x,y){
...
function animate(){
    ...
    if(frame==1 || (frame/150) % 1 == 0){
            ...
            var key=Math.floor(Math.random()*(2-0+1))+0;
            //console.log('key: ',key);
            aliens.push(new alien(yPos,key));
        }
    ...
    if(!alive){
        ...
        var hs= localStorage.getItem("SpacewarHacker");
            if(hs==null || hs<score){
                hs=score;
            }
            localStorage.setItem("SpacewarHacker",hs);
            document.getElementById('highsc').textContent = 'HighScore: '+hs;
    }
    else{
